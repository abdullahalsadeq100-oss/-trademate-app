...
/* ---------------------------------------------------------
   TradePlaza Ireland, live version (Supabase backend)
...
TradePlaza Ireland is a booking and quoting tool that connects customers with independent tradespeople. TradePlaza is not itself a tradesperson, does not employ or supervise the tradespeople listed, and is not a party to any agreement between a customer and a tradesperson.

WHO WE ARE
TradePlaza provides the platform only. All quotes, bookings, work performed, and payments are agreements directly between the customer and the tradesperson.
...
PAYMENTS
Card payments are processed by Stripe. TradePlaza does not store card details. Disputes about the quality or completion of work are between the customer and the tradesperson; TradePlaza does not adjudicate these disputes.

LIMITATION OF LIABILITY
TradePlaza is provided "as is." To the maximum extent permitted by law, TradePlaza is not liable for the acts, omissions, or work quality of any tradesperson listed on the platform, or for any loss arising from use of the platform.
...
- Payment information is collected and processed directly by Stripe, TradePlaza does not see or store your card details.
...
function downloadInvoicePdf(lead, businessName) {
  ...
  doc.setFontSize(20);
  doc.text(businessName || "TradePlaza", 14, 20);
  doc.setFontSize(10);
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text("Generated by TradePlaza Ireland", 14, 285);
...
function LegalLinks({ onLegal, dark }) {
  return (
    <div className={dark ? "text-[11px]" : "text-center mt-6 text-[11px]"} style={{ color: dark ? "rgba(255,255,255,0.6)" : "#8b8474" }}>
...
function RoleSelect({ onPro, onCustomer, onBrowse, onLegal }) {
  ...
  const serviceGroups = [
    { label: "Home & trades", items: ["Plumbing", "Electrical", "Painting & decorating", "Carpentry & joinery", "Roofing", "Tiling", "Plastering", "Heating & HVAC", "Locksmith", "Appliance repair", "Flooring installation", "Kitchen fitting", "Bathroom fitting", "Handyman / general repairs", "Window & door fitting"] },
    { label: "Outdoor & property", items: ["Gardening & landscaping", "Driveways & paving", "Fencing & gates", "Chimney sweeping", "Pest control", "Removals & man with a van"] },
    { label: "Personal & wellness", items: ["Massage therapy", "Personal training", "Hairdressing & barbering", "Beauty & nail services"] },
    { label: "Life & everything else", items: ["Catering & private chef", "Photography", "Tutoring", "Dog grooming & walking", "Event planning", "Car mechanic", "IT & computer repair", "Interior design", "Home cleaning"] },
  ];
  return (
    <div>
      {/* Nav */}
      ...
      <div className="max-w-5xl mx-auto px-4 pt-4 pb-14 grid md:grid-cols-2 gap-10 items-center">
        <div>
          ...
          <p className="mt-4 text-xs" style={{ color: "#8b8474" }}>Free to browse. No account needed until you're ready to book.</p>
        </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-14">
        ...
        <div className="relative">
          <div className="absolute left-[11px] top-2 bottom-2 w-px" style={{ background: "#e3dbc8" }} />
          <div className="space-y-7">
            {steps.map((s) => (
              <div key={s.n} className="flex gap-4 relative">
                <div style={{ background: "#F6F1E7", border: "1.5px solid #FF6A13", color: "#FF6A13" }} className="tm-mono text-xs shrink-0 w-6 h-6 rounded-full flex items-center justify-center z-10">{s.n}</div>
      ...
      <div style={{ background: "white", borderTop: "1px solid #e3dbc8", borderBottom: "1px solid #e3dbc8" }} className="py-14">
        <div className="max-w-3xl mx-auto px-4">
          ...
          <p className="text-sm mb-8" style={{ color: "#5B6B7D" }}>From emergency call-outs to a weekly massage, if it's a real skill, someone on TradePlaza probably offers it.</p>
          <div className="grid sm:grid-cols-2 gap-x-8 gap-y-6">
            {serviceGroups.map((g) => (
              <div key={g.label}>
                <div className="text-xs font-semibold uppercase tracking-wide mb-2.5" style={{ color: "#8b8474" }}>{g.label}</div>
      ...
      {/* Trust */}
      <div className="max-w-3xl mx-auto px-4 py-14">
        <h2 className="tm-display text-xl mb-8" style={{ color: "#10233B" }}>Built so you can trust who shows up</h2>
        <div className="grid sm:grid-cols-2 gap-8">
          <div className="flex items-start gap-3">
            <div className="shrink-0 mt-0.5"><VerifiedBadge /></div>
            <div>
              <div className="font-semibold text-sm mb-1" style={{ color: "#10233B" }}>Ask to see the badge</div>
              <p className="text-sm" style={{ color: "#5B6B7D" }}>Professionals can submit their trade licence and insurance for review. Once approved, their profile shows a Verified badge.</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="shrink-0 mt-0.5"><Euro size={18} color="#2F8F5B" /></div>
            <div>
              <div className="font-semibold text-sm mb-1" style={{ color: "#10233B" }}>Payment stays protected</div>
              <p className="text-sm" style={{ color: "#5B6B7D" }}>Pay securely through TradePlaza once the job's confirmed. Every job has a proper invoice, not a cash-in-hand guess.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Footer */}
      <div style={{ background: "#10233B" }} className="py-10">
        ...
        <div className="p-4 space-y-3">
          ...
          <div style={{ borderTop: "1px dashed #d8d0bd" }} className="pt-3">
            <Field label="Add a team member">
              <p className="text-[11px] mt-1 mb-1" style={{ color: "#8b8474" }}>They need a TradePlaza account already (any email that's signed up before), this gives them full access to your jobs.</p>
              <div className="flex gap-2">
